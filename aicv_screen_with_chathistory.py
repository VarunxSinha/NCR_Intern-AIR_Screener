# streamlit run aicv_screen_v2.py 
...
cssstyle = "<style> [theme] primaryColor="#FF4B4B" secondaryBackgroundColor="#004E42" textColor="#000000" font="sans serif" </style>"


st.markdown(cssstyle,unsafe_allow_html=True)

with st.sidebar:
  st.markdown("""<span style='color: white;font-size: 25px;'>Welcome to - AI Based Resume Screening Chatbot </span>""", unsafe_allow_html=True)
  st.caption("A chatbot powered by Azure OpenAI")
  #st.markdown("""<span style='color: white;font-size: 15px;'>Progress Bar</span>""", unsafe_allow_html=True)
  #bar = st.progress(0)
...
res_screenresult_ques = (
    "Screen all top candidates who closely match the job description Job Description and give top 2. For each candidate, display their name, years of experience, education, highest degree, AI rating (on a scale of 1 to 10) and a brief resume summary ONLY in tabular format. Job Description is - " + jd +"display in descending order of ai rating"
)
